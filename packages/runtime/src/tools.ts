...
import { collectJob, guardJob, cacheJob, type JobHooks } from './lib/jobs.js';
...
export { DESCRIPTIONS };
export async function runTool(name: string, input: unknown, hooks: JobHooks = {}): Promise<{ ok: boolean; text: string }> {
  const started = performance.now();
  try {
    const tool = TOOLS.find(t => t.name === name);
    if (!tool) throw new Error(`Unknown tool. Choose ${TOOLS.map(t => t.name).join(', ')}.`);
    const result = tool.name === 'collect_pages' ? await collectJob(input, hooks) : await tool.run(input);
