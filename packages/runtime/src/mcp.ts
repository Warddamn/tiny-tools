...
for (const tool of TOOLS) server.registerTool(tool.name, { title: tool.title, description: DESCRIPTIONS[tool.name], inputSchema: tool.schema.shape, annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: tool.openWorld } }, async (args: unknown) => {
