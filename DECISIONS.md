...
- **2026-09-23 — discover actual capabilities, not just a name:** publish task examples plus a JSON catalog generated from real MCP initialization/tool-list responses and fail CI on drift. The catalog is project-specific and never claims automatic installation. Replace stale npm prerequisites and list only built products.
- **2026-09-23 — reproducible directory inspection:** Glama's profile has no inspected tools and cannot deploy; provide an explicit Docker build instead of relying on inferred workspace setup. Default context and separate runtime targets preserve tool budgets and dependency separation. Test all tools in non-root, network-disabled containers; third-party indexing remains unverified until observed. No tool-code/version bump or release re-download is needed for this discovery update.
