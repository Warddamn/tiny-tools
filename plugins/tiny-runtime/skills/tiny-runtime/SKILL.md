---
name: tiny-runtime
description: Use for configured cursor-paginated API or local batch jobs requiring complete collection, exact integer totals, duplicate checks or checkpoint resumption; repeated-failure traces with measured state; and tool-progress traces for cache scheduling. Do not use for one simple request, small notes, an existing correct script, or ordinary hosted-model conversations. Trace tools analyze evidence; automatic guards and GPU effects require host integration.
---
...
See the package README for JSON source mappings and trace formats. Never invent completion, state fingerprints, GPU changes or token savings.
